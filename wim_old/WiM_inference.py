...
from RL_marin_generation import RLMarginGenerator
...
def run_wim_rl(
    ...
    # # Initialize WIM KV cache with a dummy input to ensure it's properly set up
    # dummy_input = "This is a dummy input to initialize KV cache."
    # try:
    #     print('here')
    #     wim_inference.prefill_text_with_kv_cache(dummy_input, wim_inference.wim_kv_cache)
    #     print(f"Dummy input prefilled for WIM KV cache: {wim_inference.wim_kv_cache.get_seq_length()}")
    #     wim_inference.prefill_text_with_kv_cache(dummy_input, wim_inference.classifier_kv_cache)
    #     print(f"Dummy input prefilled for WIM KV cache: {wim_inference.classifier_kv_cache.get_seq_length()}")
    #     print("end")
    # except Exception as e:
    #     print(f"Error during dummy input prefill: {e}")
    # # Now clear the caches
    # wim_inference.shrink_kv_cache_from_end(0, wim_inference.wim_kv_cache)
    # wim_inference.shrink_kv_cache_from_end(0, wim_inference.classifier_kv_cache)
    ...
    if use_rl_generator or train_rl_generator:
        ...
        rl_margin_generator = RLMarginGenerator(
            model_id=model_id,
            reward_model_id=model_id,  # Use same model for rewards
